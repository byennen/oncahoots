class ApplicationController < ActionController::Base
  ...
  def load_university_data
    if @university
      ...
      @free_food_events = @university.events.search_date(Date.today).free_food.order(:at_time).reverse_order + 
           @university.club_events.search_date(Date.today).free_food.order(:at_time).reverse_order
